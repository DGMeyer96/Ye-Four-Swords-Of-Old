...
const COMBO_SPEED = 0.9

var attackCombo = 0
var timeSinceLastAttack = ATTACK_SPEED
...
func _physics_process(delta):
	...
	if timeSinceLastAttack > ATTACK_SPEED:
		...
		elif isAttacking && attackCombo == 1 && timeSinceLastAttack < COMBO_SPEED:
			_animation_tree.set("parameters/conditions/isAttacking2", true)
			timeSinceLastAttack = 0.0
			attackCombo = 2
		# Full combo chain reached, reset
		elif timeSinceLastAttack > COMBO_SPEED || attackCombo > 1:
			attackCombo = 0
